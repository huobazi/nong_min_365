...
desc 'Create some config files from the example files ,database.yml newrelic.yml secret_token.rb, etc.' 
task :create_app_configs do
  files_hash = {
    "config/database.yml.example"                 => "#{deploy_to}/#{shared_path}/config/database.yml",
    "config/newrelic.yml.example"                 => "#{deploy_to}/#{shared_path}/config/newrelic.yml",
    "config/initializers/secret_token.rb.example" => "#{deploy_to}/#{shared_path}/config/initializers/secret_token.rb",
  }
  files_hash.each do |source, target|
    upload_file "the #{source}", source, target 
    queue 'echo "-----> PLEASE edit the -----> #{target} !!!"'
  end
end
